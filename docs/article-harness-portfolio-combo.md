...
我把文章生成设计成一条可控工作流，而不是一次性 prompt。系统先统一素材，再规划结构，接着生成 Markdown 主稿，最后渲染成 HTML，并通过双层 review 保证内容和展示都不跑偏。
...
我把“生成一篇文章”从一次性 prompt，改造成了一套可控、可追踪、可修复的内容生产系统。
